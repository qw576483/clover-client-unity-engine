...
// 「格集合 ↔ base64 位图」的编解码：**逐格幂等 + 越界容错 + 坏串不抛**。
//
// 定位：把一份体积大的格集合（小地图已探索格这类）压成 base64 位图落盘、读档再解回来。
//   编解码**只跟 `w/h/格索引` 有关**（纯 `System` / `System.Text`，零 Unity、零业务语义）⇒
//   任何"稀疏布尔格 / 已访问区域 / 迷雾 / 掩码"都能用同一份实现。
//   调用方保留自己的 DTO（区域 id 等**语义字段**）与语义命名，转调本件。
//
// 语义约束（改一条 = 语义漂移；**逐字节确定性**是硬要求）：
