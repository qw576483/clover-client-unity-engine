...
// 乱序异步结果按下标落位 —— 通用横切能力，下沉到引擎。
//
// 来源：从**游戏项目的项目内 helper 收敛而来**（同一个形状在多个项目 / 多个功能里被各自手写过一遍：
//   并行加载 N 份资源、逐帧确定性回放收集 N 帧结果、N 个子请求的结果汇总）。
//   收敛的依据是它解决的问题处处相同，而不是某一款游戏的领域内容。
//
...
namespace CloverEngine
{
    ...
    /// </para>
    /// <para>
    /// 来源：从游戏项目的项目内 helper 收敛而来（并行加载 / 逐帧采集两处各写过一遍同一形状）。
    /// </para>
    /// </summary>
