...
// 注入式可复现随机（**seed 可复现**）—— 通用横切能力，下沉到引擎。
//
// 出处：Diablo2 项目 `client/Assets/Scripts/Core/Rng.cs`（178 行，整类语义照搬）。
//   唯一改写：项目日志门面 `Log.WarnThrottled` ⇒ 引擎 `LogThrottle.WarnThrottled`、
//   `Log.Info` ⇒ `Game.Logger.Info`（两套门面同源，见 Runtime/Core/LogThrottle.cs）。
//
// 为什么下沉：随机器是**任何项目都要的底座**（地图生成 / 掉落 / 洗牌 / 刷怪），
//   属 `结构规则.md` §2.2「新增模块判定五问」第 1 问（跨模块通用底座 ⇒ Core）。
//   各项目各写一份的代价已被验证：同一个「非法参数处理 / 降频告警」的坑会被重踩 N 次。
//
