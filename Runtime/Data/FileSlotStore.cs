...
// 「键 → 文本」的文件**槽位**存储：原子写 + 损坏留档 + 枚举 —— 通用横切能力，下沉到引擎。
//
...
//   ⇒ 本类把这一层下沉到引擎。与 `Setting` **互补**、不改它：`Setting` = 单文件 KV（设置在内存里攒、
//   `Save()` 一次写全），本类 = **一槽一文件**（每次 `Write` 独立落盘、`List()` 可枚举）。
//
// 原子写那一段是**照搬 `Setting` 的成熟写法**（同目录同族，⛔ 不另创一套）：
...
//     维护一个「索引键」（例如 `char/index`）来记住有哪些槽 —— 把槽位清单交给本类就不必维护索引。
...
namespace CloverEngine
{
    ...
    public sealed class FileSlotStore
    {
        ...
        // ── 内部：原子写 / 留档（照搬 `Setting` 的写法，见文件头出处） ─────────
