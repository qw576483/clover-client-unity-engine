...
// 相机边界夹制（**格空间**，⛔ 不是世界 AABB）的纯函数 —— 能力下沉自
// clover-project-diablo2 的 client/Assets/Scripts/Module/Camera/CameraBounds.cs
// （算法与每一个边界分支**逐行同源**；原处已改为薄转发，⛔ 不留第二份）。
//
// 为什么必须夹到格空间（原处实测根因，逐字保留）：
...
//   修法 = 夹制回到**格空间**：把「可见格矩形」夹进 [0..W]×[0..H]（连续格坐标口径）。
//
...
namespace CloverEngine
{
    ...
    /// 改了生产也不变红的假闸门）。
    /// <para>唯一的实现处：<c>clover-project-diablo2</c> 的
    /// <c>Module/Camera/CameraBounds.cs</c>（薄转发）与 <c>CameraRig.CameraPosForCamera</c> 都走这里。</para>
    /// </summary>
