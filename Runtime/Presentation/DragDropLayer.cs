...
//
// 来源（口径逐字搬移；只搬机制，**不搬任何项目语义**）：
//   clover-project-diablo2 · client/Assets/Scripts/UI/InventoryPanel.cs（改前 2026-09-24）
//     · :447-454  拖影节点 `DragGhost` + 目标格高亮节点 `DropHighlight`（各建一次、复用之）
//     · :846-884  `OnDrag`：拖影贴在指针上（`rectTransform.position = 鼠标屏幕点`）+
//                 指针下的格亮起 / 离开则熄灭（**只在状态变化时报一次**日志）
//     · :887-929  `OnEndDrag`：先收表现（藏拖影 / 熄高亮）→ 再判落点 → 再发请求
//     · :1042-1049 `CellAt(Vector2 screen)`：屏幕点 → 格下标（= 本件「屏幕点 → 网格坐标」）
//     · :935-958  `PlanDrop(...)`：★ **留在项目侧**（判"装备槽 / 背包内移动 / 丢地面"是 D2 语义）
//   屏幕点换算复用的既有件：`Runtime/Core/ScreenPointUtil.cs`（画布模式取相机的唯一口径，
//     含"Overlay 画布世界坐标就是屏幕像素"的根因记录）。
//
// 为什么沉：
//   「列表/格盘里的东西可以被拖到另一个格上」是**通用** UI 结构：每个项目都会重写一遍
//   "取拖影 → 跟随指针 → 屏幕点换算成格 → 高亮目标格 → 松手判落点"，而**只有最后一步的
//   业务规则**（哪些格可放 / 装备 vs 背包 / 堆叠 / 丢地面）是各项目自己的。
//   ⇒ 本件把前四步收敛成一处；业务只提供两样东西：① 网格几何（几列几行 / 格大小 / 格区左上角）
...
//     后者由业务注入；`CanPlace == null` ⇒ 一律按可放处理 + 降频 Warn（"未接线"必须能查）。
...
namespace CloverEngine
{
    ...
    public sealed class DragDropLayer
    {
        ...
        /// <para>
        /// <c>null</c> ⇒ 一律按**可放**处理 + 降频 Warn（"未接线"必须能查，⛔ 不静默拦下也没有静默放行）。
        /// </para>
        /// </summary>
        ...
        /// <para>
        /// ★ **只判几何 + 可放性**，不含任何游戏语义；D2 的"装备槽 / 背包内移动 / 丢地面"仍由
        /// 调用方自己的纯函数判（见本文件头的来源条目）。
        /// </para>
        /// </summary>
