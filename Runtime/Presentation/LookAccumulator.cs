...
namespace CloverEngine
{
    ...
    /// <code>
    /// // PlayerMotor.cs（原实现，逐字）
    /// var degPerCount = _sensitivity * CsCombatTuning.DegreesPerMouseCount;      // 灵敏度 = 每 count 多少度
    ...
    /// 玩家设置里的灵敏度乘上玩法自己的换算系数后传入（原项目 = 设置值 × 0.022°/count）。
    ...
    /// 属限位口径 ⇒ 由调用方传入已夹紧的值（原项目 <c>ForceLook</c> 的 <c>Mathf.Clamp</c> 保留在调用点）。
    /// 非线程安全（主线程使用）。</para>
    /// </summary>
