...
namespace CloverEngine
{
    ...
    internal static class CloverMapFormat
    {
        ...
        /// AI 路线锚点这些"按名字取点"的东西，过去只能由项目另发一份旁路文件 + 自写解析器
        /// （cs16 的 <c>Resources/MapData/de_dust2_markers.bytes</c>），于是同一个空间事实有了两份载体、
        /// 两套解析、两处会漂移。本段把「名字 + 坐标」并进**同一份字节**（客户端与服务端读同一个文件）。
        /// </para>
        /// <para>
