...
//
// 语义：
//   · **同一路径只加载一次**（后来的调用只登记回调，就绪后统一触发）；
//     **结算只发生一次**；**缺帧不静默**（点名条带路径 + 缺口数）；**就绪后的回调可能是同帧同步的**
//     （缓存命中 / 整条 `LoadAll` 命中）⇒ 调用方必须能接受同步回调。
//   · 帧表形状：**一条横排条带 = 一张多 Sprite 贴图，子 sprite 名 = `{条带文件名}_{帧号}`**
//     （前缀 = 条带文件名 + `"_"`，尾段用 `int.TryParse` 取帧号）。
//
// 次序（⛔ 不许颠倒）：
//   ① `Game.Res.LoadAsset<Sprite>("…/button_wide_0")`（**逐帧按名**）在一张 PNG 被切成多个子
//      Sprite 的导入设置下**返回 null** —— 而 `LoadAll<Sprite>("…/button_wide")`
//      （**整条取**）拿得到 ⇒ **整条取 = 主路、逐帧按名 = 兜底**（逐帧按名必然取不到图、却让资源模块
//      白刷每帧一条 `[Error] [Resource] 加载失败`；逐帧按名**原地保留**，真取不到时照样走）。
...
namespace CloverEngine
{
    ...
    public sealed class SpriteStripLoader
    {
        ...
        /// 静默塞进末尾会让调用方拿到错帧）。同帧号**先到先得**（后到的覆盖前一个）。
        /// </para>
        /// </summary>
