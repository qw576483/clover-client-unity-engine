...
// 可变长文本的**单行显示截断**（二分找最长可行前缀 + 省略号）—— UI 通用件。
//
// 入口与各自的口径：
//     · `Ellipsis = "\u2026"`（U+2026；⛔ 不用三个 ASCII 点 —— CJK 字体下宽度不一致）
//     · `Clamp(Text label, string raw)`（装得下就原样写；装不下二分截断 + 保留省略号）
//     · `Measure(Text label, string text)`（自建 `TextGenerator` 量单行宽）
//     · `ClampSelf(Text label)`（把标签**当前**文本按同规则裁一次）
//     · `Path(Text label)`（日志用节点路径）
//
// ★ 设计依据：
...
//   同一帧里读到的还是**上一串文本**的宽度（实测：3 字串 `rectW=58 / preferredW=44`
...
//
// 注意：
//   · ⛔ **不许用改字号来"糊过去"**（改字号会让同一列标签字号不一致，且仍然可能溢出）。
//     装得下就原样写入 —— ⛔ 不无端加省略号。
