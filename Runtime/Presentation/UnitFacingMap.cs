...
//
// 来源（算法逐字搬移，**档位数已参数化**）：
//   clover-project-cr · client/Assets/Scripts/View/UnitAnimTable.cs
//     · :66    `StepCount = 16`（16 档 = 全周 360° ÷ 22.5°）
//     · :73    `StepToView = { 1,2,3,4,5,6,7,8,9,8,7,6,5,4,3,2 }`
//              （**唯一权威副本** = 该项目 `tools/probes/d129c-view-map.tsv`，由
//               `d129b-gen-anim-table.py` 逐字复制成那个常量；正确性由 `d129c-yaw-verify.py`
//               断言 A1–A6 + `--corrupt` 负控判红保证 —— 即：该表**有判据**，不许肉眼改）
//     · :76    `StepFlip(step) => step > 8`（西半边 = 同一视角 + 水平镜像）
//     · :82-88 `ViewForHeading(headingDeg, out view, out flip)`
//                `step = round((90° − φ) / 22.5°) mod 16`
//   消费方 = 同项目 `View/UnitView.cs:497-533`（`UpdateFacing`）；其滞回常量
//     `ViewHysteresisSteps = 0.25 档`（`:465`）与噪声门槛 `FacingMinMoveTiles = 0.01 格`（`:481`）。
//
// 为什么沉：
//   「朝向 → 视角号 + 是否镜像」是**多视角 2D sprite**（16 / 32 向素材）的通用问题，与具体玩法无关。
//   在原项目里，它只是那张**项目专有的帧段数据表**上的四个静态成员 ⇒ 别的项目要用就得整张数据表一起抄。
//   本件把它抽成**只依赖「档位数 + 映射表」**的独立件：映射表可换，16 档只是默认。
//
// 已知事故 / 坑（照抄原件的实测记录，⛔ 别再踩）：
//   · **档位 / 视角搞反不会报错**，只会"人物朝向看着别扭" ⇒ 映射表只能靠断言 + 负控保证，肉眼判不了。
//   · 原项目旧表曾把 9 个视角的 clip **取并集**，运行时依次播 9 个视角 ⇒ 现象 = "苍蝇海 1 秒 9 次
//     视角、走路原地打转 / 抽搐"（`策划/单位帧段表.md:2717-2718`，处置建议 = 按 clip 播）。
//     本件只回答"**该选哪一个视角**"，⛔ 不负责 clip 内容 —— "每档只取一个视角"是调用方（帧段表）的约束。
//   · 朝向极角必须由**插值窗口两端快照之差**求，⛔ 不能用逐帧位移：服务端位置是毫格量化的，
//     站立单位会被 ±2 毫格噪声把方向翻 180°（原项目 D134 实测 `id=320` 的 vstep 片段
//     `[4,12,12,12,4]`，而同期逐帧位移仅 `-0.002 格`）。
//     本件**只做映射**；滞回与噪声门槛是调用方策略（原项目取 0.25 档 / 0.01 格），⛔ 引擎不替它定死。
...
namespace CloverEngine
{
    ...
    /// ③ 镜像档 = <c>step ≥ FlipFromStep</c>（默认 <c>StepCount/2 + 1</c> = 16 档时的 9 ⇒ 与原项目
    ///    <c>step &gt; 8</c> 逐字等价）；
    ...
    public sealed class UnitFacingMap
    {
        ...
        /// <summary>
        /// 标准 16 档 → 9 视角表（**逐字搬自原件**，见文件头「来源」；权威副本是该项目的
        /// `tools/probes/d129c-view-map.tsv`，改动必须同步改表并重跑其断言 + 负控）。
        /// </summary>
        ...
        /// 换成"四舍五入远离零"会在恰好落在档边界的朝向上差一档（表现为偶发"朝向跳一格"）。
        /// </para>
        /// </summary>
