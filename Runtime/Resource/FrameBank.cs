...
//
// 本件覆盖"目录抓帧 + 帧序 + 锚点 + 生命周期"这一段，⛔ 不含任何单位 / 玩法语义。
//
// ① 帧名 → 帧号解析（`ParseFrameIndex`）：**从倒数第二段起逐段回退**找纯数字段
//    （`frame_000_0` ⇒ `000`），只有一段数字时（`frame_003` / `gen_frame_012`）才认最后一段。
//    Unity 对"一张 PNG 多个子 Sprite"的导入会把 Sprite 命名为 `<文件名>_<子序号>`；
//    若取"名字**尾部**的数字串"，尾段子序号会让整目录拿到同一个排序键 ⇒ 排序空转 ⇒
//    播放顺序退化，且**不报任何错**（最难查的一类静默失效）。
//
...
//    `fN→idx` 作为映射规则证据）。
//
// ③ 统一画布锚点（`UnifyCanvasAnchor`）：多 Sprite 导入给出的 pivot 是**每帧自己那张裁剪框的中心**
//    ⇒ 逐帧换帧时整帧被重新居中，每换一帧人物就在画布上跳一下。
//    做法：取**全目录 `rect` 并集的中心**当锚点，
//    用"相对各自 `rect` 归一化"的 pivot 重建每一帧 ⇒ 每帧按美术自己的画布位置落位，换帧不位移。
//    （`Sprite.Create` 的 `pivot` 参数口径是**相对 `rect` 归一化**、不是相对整张贴图：
//     `pivot = ((anchorX - rect.x) / rect.width, (anchorY - rect.y) / rect.height)`。）
//
...
namespace CloverEngine
{
    ...
    public sealed class FrameBank
    {
        ...
        public enum PivotMode
        {
            ...
            /// 因此不同帧之间**固有的落脚线差异**（各帧 `rect.y` 的分布）仍按美术原样保留，
        ...
        /// 解析不出时返回 <c>-1</c>（⛔ 不抛）。
        /// <para>
        /// <b>为什么不是"尾部数字串"</b>（见类注释①）：
        /// Unity 对"一张 PNG 多个子 Sprite"的导入会把 Sprite 命名成 <c>&lt;文件名&gt;_&lt;子序号&gt;</c>
        /// （形如 <c>frame_000_0</c>）—— 尾部那段数字是**子序号**，整目录恒同值
        /// ⇒ 所有帧拿到同一个排序键 ⇒ 排序空转 ⇒ 播放顺序变成"批量加载的偶然顺序"。
        /// 取倒数第二段（`000`）才是真正的帧序号。
        /// </para>
        /// <para>
        ...
        private Sprite[] LoadViaTexture(string path)
        {
            ...
            LogThrottle.WarnOnce(Tag, "FrameBank.TextureFallback[" + _generation + "]/" + path,
                ...
                "（素材导入类型不是 Sprite 时走本条兜底，⛔ 不是资源没加载出来）");
            return sprites;
        }
